...
class SpotActionsCfg:
    ...
    agentHXY = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=["^.*_h[xy]$"], scale=0.2, use_default_offset=True
    )
    agentKN = mdp.JointPositionActionCfg(
        asset_name="robot", joint_names=["^.*_kn$"], scale=0.2, use_default_offset=True
    )
...
class SpotObservationsCfg:
    ...
    class AgentHXYCfg(ObsGroup):
        ...
        my_actions = ObsTerm(func=mdp.last_action, params={"action_name": "agentHXY"})
    ...
    class AgentKNCfg(ObsGroup):
        ...
        my_joint_pos = ObsTerm(
            func=mdp.joint_pos_rel,
            params={"asset_cfg": SceneEntityCfg("robot", joint_names=["^.*_kn$"])},
            noise=Unoise(n_min=-0.05, n_max=0.05),
        )
        other_joint_pos = ObsTerm(
            func=mdp.joint_pos_rel,
        ...
        my_actions = ObsTerm(func=mdp.last_action, params={"action_name": "agentKN"})
    ...
    # observation groups
    agentHXY: AgentHXYCfg = AgentHXYCfg()
    agentKN: AgentKNCfg = AgentKNCfg()
...
@configclass
class SpotFlatFactEnvCfg(LocomotionVelocityRoughEnvCfg):
...
class SpotFlatEnvCfg_PLAY(SpotFlatFactEnvCfg):
