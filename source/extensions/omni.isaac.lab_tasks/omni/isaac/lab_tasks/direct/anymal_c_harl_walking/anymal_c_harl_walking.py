...
@configclass
class AnymalCHarlWalkingFlatEnvCfg(DirectMARLEnvCfg):
    # env
    ...
    observation_spaces = {f"robot_{i}": 235 for i in range(1)}
    state_space = 0
...
@configclass
class AnymalCHarlWalkingRoughEnvCfg(AnymalCHarlWalkingFlatEnvCfg):
    # env
    observation_space = 235
...
class AnymalCHarlWalking(DirectMARLEnv):
    cfg: AnymalCHarlWalkingFlatEnvCfg | AnymalCHarlWalkingRoughEnvCfg

    def __init__(self, cfg: AnymalCHarlWalkingFlatEnvCfg | AnymalCHarlWalkingRoughEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_observations(self) -> dict:
        ...
        for robot_id, robot in self.robots.items():
            height_data = None
            # if isinstance(self.cfg, AnymalCHarlWalkingWalkingRoughEnvCfg):
            height_data = (
    ...
    def _get_rewards(self) -> dict:
        ...
        for robot_id, _robot in self.robots.items():
            ...
            ang_vel_error = torch.sum(torch.square(_robot.data.root_com_ang_vel_b[:, :2]), dim=1)
            # joint torques
            joint_torques = torch.sum(torch.square(_robot.data.applied_torque), dim=1)
            # joint acceleration
            joint_accel = torch.sum(torch.square(_robot.data.joint_acc), dim=1)
            # action rate
            ...
            # flat orientation
            flat_orientation = torch.sum(torch.square(_robot.data.projected_gravity_b[:, :2]), dim=1)
    ...
    def _get_dones(self) -> tuple[dict, dict]:
        ...
        return {key:time_out for key in self.robots.keys()}, {key:dones[i]  for i, key in enumerate(self.robots.keys())}
